...
/**
 * Export the BEP as a buildingSMART Information Delivery Specification (IDS) XML
 * file. IDS (v1.0, June 2024) expresses information requirements machine-readably
 * against an IFC model. This is a pragmatic, standards-aligned export: each BIM
 * use / information exchange becomes an IDS specification, and LOD matrix rows
 * become property requirements. Per the research: JSON is the internal canonical
 * model; IDS XML is the buildingSMART-interoperability export.
 */
export function bepToIds(doc: BepDocument): string {
  const x = (s: string) =>
    s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  const lines: string[] = [];
  lines.push(`<?xml version="1.0" encoding="UTF-8"?>`);
  lines.push(
    `<ids xmlns="http://www.buildingsmart.org/ids" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="0.9.7" xsi:schemaLocation="http://www.buildingsmart.org/ids https://raw.githubusercontent.com/buildingSMART/IDS/master/IDS.xsd">`,
  );
  lines.push(`  <info>`);
  lines.push(`    <title>${x(doc.projectName)} — BIM Execution Plan (${doc.mode})</title>`);
  lines.push(`    <copyright>Generated by BIM Execution Plan Studio</copyright>`);
  lines.push(`    <version>${x(doc.documentControl.revision)}</version>`);
  lines.push(`    <date>${doc.updatedAt.slice(0, 10)}</date>`);
  lines.push(`  </info>`);
  lines.push(`  <specifications>`);

  // One specification per BIM use.
  for (const u of doc.bimGoals.uses) {
    lines.push(`    <specification>`);
    lines.push(`      <info>`);
    lines.push(`        <name>${x(u.name)}</name>`);
    lines.push(`        <instructions>BIM use phase: ${x(u.phase)}; priority: ${u.priority}; responsible: ${x(u.responsibleParty)}</instructions>`);
    lines.push(`      </info>`);
    lines.push(`      <requirements>`);
    lines.push(`        <applicability>`);
    lines.push(`          <entity>`);
    lines.push(`            <name>`);
    lines.push(`              <simpleValue>IfcProduct</simpleValue>`);
    lines.push(`            </name>`);
    lines.push(`          </entity>`);
    lines.push(`        </applicability>`);
    lines.push(`      </requirements>`);
  ...
  // One specification per information exchange.
  for (const e of doc.dataExchange.exchanges) {
    lines.push(`    <specification>`);
    lines.push(`      <info>`);
    lines.push(`        <name>${x(e.name)}</name>`);
    lines.push(`        <instructions>Recipient: ${x(e.recipient)} — format ${x(e.format)} (${x(e.formatVersion)}) — LOD ${x(e.levelOfDetail)} — author ${x(e.responsibleAuthor)}</instructions>`);
    lines.push(`      </info>`);
    lines.push(`      <requirements>`);
    lines.push(`        <applicability>`);
    lines.push(`          <entity>`);
    lines.push(`            <name>`);
    lines.push(`              <simpleValue>IfcBuildingElement</simpleValue>`);
    lines.push(`            </name>`);
    lines.push(`          </entity>`);
    lines.push(`        </applicability>`);
    lines.push(`      </requirements>`);
